...
(async()=>{
  ...
  function substituteScore(wanted,candidate,opponentTeam,wantedIndex,candidateIndex){
    ...
    // Role is a hard PvP constraint: Tank/Attacker/Support/Healer/T.S/exception are not interchangeable.
    if(!a.role||a.role!==b.role)return null;
    ...
    if(a.type==='SPECIAL'){
      ...
      return {score:Math.min(1,score),exact:false};
    }
    ...
    return score>=.42?{score,exact:false}:null;
  }
  ...
  function compareLineupSubstitute(actual,query,opponentTeam){
    ...
    const used=new Set(),replacements=[];let total=0,exact=0;
    for(const {n,i} of known){
      ...
      if(!best)return null;
      used.add(best.j);total+=best.score;if(best.exact)exact++;else replacements.push({from:n,to:best.name,score:best.score});
    }
